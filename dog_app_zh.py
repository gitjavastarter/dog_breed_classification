...
print('human_files_detect_percentage: {}%'.format(Cnt_human))
print('dog_files_detect_percentage: {}%'.format(Cnt_dog))
...
# 
# __回答:__不合理，利用Resnet50和CNN深度网络模型去实现
# 
# 
...
# 
# __回答:__ 根据上图搭建，采用卷积-池化-卷积-池化-卷积-池化-池化-全连接的步骤。采用了三层卷积神经网络，并用了池化和全联接层。根据上图提示步骤搭建的CNN,该架构相比MLP肯定更能在该问题上仅仅训练少数epoch就能有超过1%的表现。原因是相比于MLP，CNN更能考虑到图像的空间结构。CNN架构采用共享权重和偏置，能很好地适应图像的平移不变形，采用Pooling层可以简化从卷积层输出的信息的作用。相比MLP大大减少了参与的网络的参数。反过来，这将导致更快的卷积模型的训练，而且在图像识别问题上取得很好的表现。卷积层使用Relu，它就是取的max(0,x)，因为神经网络是不断反复计算，实际上变成了它在尝试不断试探如何用一个大多数为0的矩阵来尝试表达数据特征，结果因为稀疏特性的存在，反而这种方法变得运算得又快效果又好了，输出层通过softmax函数一作用，就映射成为(0,1)的值，而这些值的累和为1（满足概率的性质），那么我们就可以将它理解成概率，在最后选取输出结点的时候，我们就可以选取概率最大（也就是值对应最大的）结点，作为我们的预测目标
...
# 
# 
# __回答:__ 从迁移学习，将Xception的最后一层卷积层作为输入，应用平均池化层和全联接层做输出层。采用全局平均池化层有助于减少网络中数据的维数, 采用Flatten层将把多维的输入一维化，并不影响batch的大小,采用Dense层可扩展网络, 同时允许在整个网络中发送更多信息,输出层必须有133个节点, 因为有133个类别和 softmax 用于激活。原采用全局平均池化层用于将多维输入一维化，但是会大大损失图像的特征，因此改用Flatten层。从Flatten层到最后的输出tensor，层数应该是逐步的缩小的，因此采用2058-512-133的结构。经过多次试验发现这样参数不至于过多，最后测试准确率还可以。早期的VGG16迁移学习，使用全局平均池化层，大大损失了图像的特征；从Flatten层到最后的输出tensor，参数减少的太快。
# 
# 
...
## 自由地使用所需的代码单元数吧
predictions(human_files_short[3])
predictions(human_files_short[78])
predictions(dog_files_short[3])
predictions(dog_files_short[8])
predictions(dog_files_short[56])
predictions(dog_files_short[78])
